...
// Some Android skins (MIUI/Xiaomi and, to a lesser extent, ColorOS/Realme
// and FunTouch/Vivo) manage notification permission for an installed PWA
// separately from Chrome's own permission prompt: even after the user taps
// "Allow" (or even if Chrome never shows a prompt at all and
// Notification.requestPermission() silently resolves to "denied"), the
// system can still be blocking notifications for that specific installed
// app/icon until the user flips a switch in the phone's own Settings app.
// This can't be fixed from JS — it's a device-level restriction, not a bug
// in this app — but we can at least point the user at the right screen
// instead of showing the same generic message we'd show anywhere else.
function isLikelyMiuiDevice() {
    const ua = navigator.userAgent || '';
    return /XiaoMi|MI \d|Redmi|POCO|MIUI/i.test(ua);
}
...
async function enableNotifications() {
    ...
    if (permission !== 'granted') {
        if (isLikelyMiuiDevice()) {
            showToast('⚠️', 'روی گوشی‌های شیائومی (MIUI) معمولاً این تنظیم جدا از خود برنامه است: به تنظیمات گوشی → برنامه‌ها → مدیریت برنامه‌ها → این برنامه (یا Chrome) → اعلان‌ها بروید و آن را فعال کنید، سپس دوباره اینجا را بزنید', 'error');
        } else {
